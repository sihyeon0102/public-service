...
const endpoints = {
  ...
  C: {
    name: 'seoul_district_library',
    path: '/api/facilities?region=SEOUL&district=SEOUL-DISTRICT-1&type=LIBRARY&page=0&size=20',
    totalElements: 5000,
  },
};
...
if (!target) {
  throw new Error(`Unknown TARGET '${targetKey}'. Use A, B, or C.`);
}
...
for (const vus of [5, 10, 20, 40]) {
  stageMetrics[vus] = {
    duration: new Trend(`facility_duration_vu_${vus}`, true),
    failed: new Rate(`facility_failed_vu_${vus}`),
    requests: new Counter(`facility_requests_vu_${vus}`),
  };
}
...
function baselineScenarios() {
  ...
  for (const vus of [5, 10, 20, 40]) {
    scenarios[`load_${vus}`] = {
      ...
      gracefulRampDown: '5s',
      tags: { endpoint: target.name, load_stage: String(vus) },
    };
    startSeconds += 55;
  }
  return scenarios;
  ...
  if (mode === 'smoke') {
    return {
      smoke: {
        ...
        duration: '10s',
        gracefulStop: '5s',
        tags: { endpoint: target.name, load_stage: 'smoke' },
      },
    };
  }
  if (mode === 'warmup') {
    return {
      warmup: {
        ...
        gracefulRampDown: '5s',
        tags: { endpoint: target.name, load_stage: 'warmup' },
      },
    };
  }
...
function requestFacility(vus) {
  const response = http.get(`${baseUrl}${target.path}`, {
    tags: { endpoint: target.name, request_kind: 'facility_search' },
    timeout: '60s',
  });
...
export function setup() {
  const response = http.get(`${baseUrl}${target.path}`, {
    tags: { endpoint: target.name, request_kind: 'setup_validation' },
    timeout: '60s',
  });
